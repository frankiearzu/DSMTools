...
local MAIN_MENU_LIB_VERSION = "0.59"
...
local function Main_Send_Receive()
    ...
    elseif ctx.Phase == PHASE.VALUE_CHANGING then -- send value
        ...
        --Log.LOG_write("SEND SIM_updateMenuValue(ValueId=0x%X Text=\"%s\" Value=%s)\n", line.ValId, line.Text, menuLib.lineValue2String(line))
    ...
    elseif ctx.Phase == PHASE.VALUE_CHANGE_END then -- send value
        ...
        --Log.LOG_write("%3.3f %s: ", menuLib.getElapsedTime(), menuLib.phase2String(ctx.Phase))
        --Log.LOG_write("SEND SIM_updateMenuValue(ValueId=0x%X Text=\"%s\" Value=%s)\n", line.ValId, line.Text, menuLib.lineValue2String(line))
        --Log.LOG_write("SEND SIM_validateMenuValue(ValueId=0x%X Text=\"%s\" Value=%s)\n", line.ValId, line.Text, menuLib.lineValue2String(line))
        ctx.Phase = PHASE.WAIT_CMD
