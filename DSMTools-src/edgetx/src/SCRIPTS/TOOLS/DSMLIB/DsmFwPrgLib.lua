...
local function DSM_ReleaseConnection()
    Log.LOG_write("DSM_ReleaseConnection()\n")
    multiBuffer(0, 0)
end
...
local function DSM_getMenu(menuId, latSelLine)
    Log.LOG_write("SEND DSM_getMenu(MenuId=0x%X LastSelectedLine=%s)\n", menuId, latSelLine)
...
local function DSM_updateMenuValue(valId, val, text, line)
    ...
    Log.LOG_write("SEND DSM_updateValue(ValueId=0x%X,val=%d) Extra: Text=\"%s\" Value=%s\n", valId, val, text, menuLib.lineValue2String(line))
...
local function DSM_validateMenuValue(valId, text, line)
    Log.LOG_write("SEND DSM_validateValue(ValueId=0x%X) Extra: Text=\"%s\" Value=%s\n", valId, text, menuLib.lineValue2String(line))
...
local function DSM_editingValueEnd(lineNum, text, line)
    Log.LOG_write("SEND DSM_editingValueEnd(lineNo=0x%X) Extra: Text=\"%s\"  Value=%s\n", lineNum, text, menuLib.lineValue2String(line))
...
local function DSM_sendTxSubtrim_21(portNo)
    ...
    if (bit32.band(usage,CH_TYPE.THR)>0 or usage == 0) then 
       left = 0
       right = 2047
    end
...
local function DSM_sentTxInfo(menuId,portNo)
        ...
        elseif (TxInfo_Step == 5) then
            -- 22,4: 0 0 
            Log.LOG_write("CALL DSM_TxInfo_End(#%d)\n", portNo)
...
local function DSM_sendRequest()  
    ...
    Log.LOG_write("%3.3f %s: ", menuLib.getElapsedTime(), menuLib.phase2String(ctx.Phase))
...
local function DSM_parseRxVersion()
    ...
    Log.LOG_write("RESPONSE Receiver=%s Version %s  Cap:0x%02X\n", 
                    DSM_Context.RX.Name, DSM_Context.RX.Version, multiBuffer(18))
end
...
local function DSM_parseMenu()
    ...
    Log.LOG_write("RESPONSE Menu: %s\n", menuLib.menu2String(menu))
    return menu
end
...
local function DSM_parseMenuLine()
    ...
    if (line.Type > 0 and type == 0) then
        Log.LOG_write("RESPONSE MenuLine: ERROR. Trying to ZERO Override: %s\n", menuLib.menuLine2String(line))
    ...
    Log.LOG_write("RESPONSE MenuLine: %s\n", menuLib.menuLine2String(line))
...
local function DSM_parseMenuValue()
    ...
    if (updatedLine == nil) then
        Log.LOG_write("RESPONSE MenuValue: ERROR, Cant find Menu Line with MenuId=%X, ValID=%X to update\n", menuId, valId)
    else
...
local function DSM_parseReqTxInfo() 
    ...
    Log.LOG_write("RESPONSE ReqTXChannelInfo(#%d %s InfoType=0x%0X)\n", 
                            portNo, MODEL.PORT_TEXT[portNo], TxInfo_Type)
...
local function DSM_processResponse()
    ...
    if (cmd > 0x00) then Log.LOG_write("%3.3f %s: ", menuLib.getElapsedTime(), menuLib.phase2String(ctx.Phase)) end
    ...
    elseif cmd == 0x02 then -- read menu title
        ...
        if (ctx.Menu.MenuId == 0x0001) then  -- Still in RESETTING MENU???
            -- Star from Start
            Log.LOG_write("RX Reset:  Still not done, restart again!!!\n")
            ctx.Menu.MenuId = 0
            ctx.Phase = PHASE.RX_VERSION
        else
...
local function Message_Init()
    ...
    if (initStep == 0) then
        if (IS_EDGETX) then
            ...
            menuLib.LoadTextFromFile(MSG_FILE,14) -- 14K of mem to read the file in in one shot
            initStep=1
        else
