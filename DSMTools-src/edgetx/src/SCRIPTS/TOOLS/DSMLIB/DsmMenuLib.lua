...
local LINE_TYPE = {
    ...
    VALUE_NUM_I8_PREC2 = 0x69,  --  8 bit number, PREC2
...
function MenuLib.Value_Add(line, inc)
    Log.LOG_write("%3.3f %s: DSM_Value_Add(%s,%s)\n", 
        MenuLib.getElapsedTime(), MenuLib.phase2String(DSM_Context.Phase), inc, MenuLib.menuLine2String(line))
...
function MenuLib.Value_Default(line)
    local origVal = line.Val
    Log.LOG_write("%3.3f %s: DSM_Value_Default(%s)\n", 
        MenuLib.getElapsedTime(), MenuLib.phase2String(DSM_Context.Phase), MenuLib.menuLine2String(line))
...
function MenuLib.Value_Write_Validate(line)
    Log.LOG_write("%3.3f %s: DSM_Value_Write_Validate(%s)\n", 
        MenuLib.getElapsedTime(), MenuLib.phase2String(DSM_Context.Phase), MenuLib.menuLine2String(line))
...
function MenuLib.GotoMenu(menuId, lastSelectedLine)
    Log.LOG_write("%3.3f %s: DSM_GotoMenu(0x%X,LastSelectedLine=%d)\n", 
        MenuLib.getElapsedTime(), MenuLib.phase2String(DSM_Context.Phase), menuId, lastSelectedLine)
...
function MenuLib.PostProcessMenu()
    ...
    --Log.LOG_write("SIM RESPONSE Menu: %s\n", MenuLib.menu2String(ctx.Menu))
    ...
    for i = 0, MenuLib.MAX_MENU_LINES do -- clear menu
        ...
        if (line.Type~=0) then
            ...
            --Log.LOG_write("SIM RESPONSE MenuLine: %s\n", MenuLib.menuLine2String(line))
        end
...
function MenuLib.Init()
    ...
    LineTypeText[LINE_TYPE.VALUE_NUM_I8_PREC2] = "V_P2"
