...
-- Phase
local PH_INIT   = 0
local PH_RX_VER , PH_TITLE , PH_TX_INFO                    = 1, 2, 3 
local PH_LINES , PH_VALUES                                        = 4, 5
...
local TX_FORWARD_PROG_VER   = 0x15
...
local function int16_LSB(number) -- Less Significat byte
  local r = bit32.band(number,0xFF)
  return r
end
...
local function Dsm_to_Int16(lsb, msb) -- Componse an Int16 value
  return bit32.bor(bit32.lshift(msb,8),lsb)
end
...
local function DSM_Release()
  LOG_write("DSM_ReleaseConnection()\n")
  if (not config.simulation) then
    multiBuffer(0, 0)
  end
  gc()
...
local function DSM_Send(...)
  ...
  local hex = string.format("%02X ",arg[1])
  local len = #arg
  if (len < 6) then len = 6 end -- Always 6 bytes

  for i = 1, len do
      hex = hex .. string.format("%02X ",arg[i] or 0)
      multiBuffer(3 + i, arg[i] or 0)
  end
  multiBuffer(3, 0x70 + #arg) -- Header: length + 0x70
...
local function DSM_HandleEvent(event, rotarySpeed)
  ...
  elseif event == EVT_VIRTUAL_ENTER then -- ENTER
...
local function DSM_Send_Receive()
  ...
  if SendDataToRX==0 and multiBuffer(10) == I2C_FORWARD_PROG  then
    ...
    -- Data processed
    multiBuffer(10, 0x00)
    RXInactivityTime = getTime() + 800   -- Reset Inactivity timeout (8s, 10ms units)
    refreshDisplay      = true
  else
  ...
  if SendDataToRX == 1 then
    ...
    DSM_SendRequest()
    refreshDisplay      = true
    TXInactivityTime = getTime() + 200   -- Reset Inactivity timeout (2s, 10ms units)
  else
...
local function DSM_Display()
  ...
  if (ctx_EditLine == nil) then
    ...
    ui.drawFPSubHeader(ui.getFPBottomLineY() + 7,msg)
  end
  ...
  for i = 0, 6 do
    ...
    y = y + ui.getFPLineHeight() + 1 -- + ui.getFPLinesHeightPad()
  end     -- for
...
local function ReadTxModelData()
  ...
  MODEL.modelPath = hashName(table.name)
