local toolName = "TNS|DSM Forward Prog v0.59a (Color) |TNE"
...
local Log = assert(loadScript(DSMLIB_PATH.."DsmLogLib.lua"), "Not-Found: DSMLIB/DsmLogLib.lua")(DEBUG_ON)
...
local function GUI_Display_Boxed_Text(lineNum,x,y,w,h,text,inv, isNumber, isFloat)
  local ctx = DSM_Context
  ...
  if (isNumber) then
    if (isFloat) then txtColor = txtColor + PREC2 end
...
local function GUI_Diplay_Button(x,y,w,h,text,selected)
  GUI_Display_Boxed_Text(-1,x,y,w,h,text,selected, false, false)
end

local function GUI_ShowBitmap(x,y,imgData, samePos)
  ...
  if (bitmap~=nil) then
     if (not samePos) then y = y + 20 end  
...
local function GUI_Display_Line_Menu(lineNum,line,selected)
  ...
  if menuLib.isSelectableLine(line) then -- Draw Selectable Menus in Boxes
    GUI_Display_Boxed_Text(lineNum,x, y, LCD_W_LINE_MENU, LCD_Y_LINE_HEIGHT, line.Text,selected, false, false)
    ...
    if (line.TextId >= 0x5000) then
      -- image
      local imgData = string.format("IMG%04X.jpg|(IMG%04X.jpg)",line.TextId,line.TextId)            
      GUI_ShowBitmap(x,y, imgData, true)
      return
    elseif menuLib.isDisplayAttr(line.TextAttr,DISP_ATTR._RIGHT) then -- Right Align???
...
local function GUI_Display_Line_Value(lineNum, line, value, selected, editing)
  ...
  if not menuLib.isFlightModeLine(line) then
    if menuLib.isSelectableLine(line) then 
      ...
      GUI_Display_Boxed_Text(lineNum,LCD_X_LINE_VALUE,y,tw,LCD_Y_LINE_HEIGHT,value,selected, not menuLib.isListLine(line), line.Type==LINE_TYPE.VALUE_NUM_I8_PREC2)
          ...
          if line.Val ~= nil then
            ...
            if menuLib.isListLine(line) then    -- for Lists of Strings, get the text
              ...
              if (imgData and i == ctx.SelLine) then  -- Optional Image and Msg for selected value
                GUI_ShowBitmap(LCD_X_LINE_TITLE,LCD_Y_LINE_START, imgData, false)
              end
            end
          end -- if Line[i]~=nil
...
local function GUI_HandleEvent(event, touchState)
  ...
  if (IS_EDGETX) then
    ...
    if (event == EVT_TOUCH_TAP or event == EVT_TOUCH_FIRST) and not ctx.isEditing() then  -- Touch and NOT editing
        Log.LOG_write("%s: EVT_TOUCH_TAP %d,%d\n",menuLib.phase2String(ctx.Phase),touchState.x, touchState.y)
        local button = GUI_getTouchButton(touchState.x, touchState.y)
        if button then
          -- Found a valid line
          ctx.SelLine = button
  ...
  if event == EVT_VIRTUAL_EXIT then
    ctx.Refresh_Display=true
    Log.LOG_write("%s: EVT_VIRTUAL_EXIT\n",menuLib.phase2String(ctx.Phase))
    if ctx.Phase == PHASE.RX_VERSION then
  ...
  if event == EVT_VIRTUAL_NEXT then
    ctx.Refresh_Display=true
    Log.LOG_write("%s: EVT_VIRTUAL_NEXT\n",menuLib.phase2String(ctx.Phase))
  ...
  if event == EVT_VIRTUAL_PREV then
    ctx.Refresh_Display=true
    Log.LOG_write("%s: EVT_VIRTUAL_PREV\n",menuLib.phase2String(ctx.Phase))
  ...
  if event == EVT_VIRTUAL_ENTER_LONG then
    ctx.Refresh_Display=true
    Log.LOG_write("%s: EVT_VIRTUAL_ENTER_LONG\n",menuLib.phase2String(ctx.Phase))
  ...
  if event == EVT_VIRTUAL_ENTER then
    ctx.Refresh_Display=true
    Log.LOG_write("%s: EVT_VIRTUAL_ENTER,  SelLine=%d\n",menuLib.phase2String(ctx.Phase), ctx.SelLine)
...
local function GUI_Warning(event,touchState)
  ...
  lcd.drawText(75,250,"    OK     ", INVERS + BOLD)
  lcd.drawText(LCD_W,250,"Frankie Arzu / Pascal Langer", RIGHT)
