...
function CaptureProcessor.wakeup(I2C_ID)
  ...
  if (data ~= nil) then   
      ...
      for i=1,3 do
        ...
        this.headerHex=this.headerHex .. string.format("0x%02X%s", (data[i] or 0xFF),sep)
      end

      for i=4,19 do
          local sep = (i==19 and "") or ","
          this.debugHex=this.debugHex .. string.format("0x%02X%s", (data[i] or 0xFF),sep)
      end
...
function MainScreenProcessor.init()
  ...
  local buttonHeightPadding = 10
  local buttonWidthPadding = 20
