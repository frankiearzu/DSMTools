...
local function formHeader(form, title)
  ...
  form.addTextButton(line, {x=w-80,y=5,w=75,h=LCD_LINE_HEIGHT*1.2}, "Back", function() DefaultProcessor.close() end)
end
...
function FlightLog.paint()  
  ...
  local topY = form.height() + LCD_LINE_HEIGHT
  ...
  for iParam=0,3 do   -- A,B,L,R 
    ...
    if (val==0x4000) then  -- Active value
        ...
        lcd.font(FONT_STD)
        showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, val)
    end

    y = y + LCD_LINE_HEIGHT + 4
  end
  ...
  for iParam=4,5 do  -- F, H
    ...
    showNumberBoxed(LCD_COL2_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT * 2, this.QOS_Value[iParam])
    y = y + (LCD_LINE_HEIGHT + 4) * 2
  end
  ...
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, bat, "v")
end
...
function AS3XSettings.paint(page)
  ...
  local y = form.height() + LCD_LINE_HEIGHT
  ...
  showNumberBoxed(x,y,LCD_SMALL_NUMBER_W,LCD_LINE_HEIGHT,this.AS3X_FmMsg)

  x = x + LCD_NUMBER_W 
  lcd.drawText (x,y, "Flags")
  x = x + lcd.getTextSize("Flags ")
  showNumberBoxed(x,y,LCD_SMALL_NUMBER_W,LCD_LINE_HEIGHT,this.AS3X_Flags)

  x = x + LCD_NUMBER_W 
  lcd.drawText (x,y, "State")
  x = x + lcd.getTextSize("State ")
  showNumberBoxed(x,y,LCD_SMALL_NUMBER_W,LCD_LINE_HEIGHT,this.AS3X_State)
  ...
  if (page==1) then
    ...
    y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
    lcd.drawText (LCD_COL1_HEADER,y, "Roll")
    lcd.font(FONT_STD)
    showNumberBoxed(x_data1,y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[0]) -- Roll G
    showNumberBoxed(x_data2,y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[3]) -- Roll H 

    y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
    lcd.font(FONT_BOLD)
    lcd.drawText (LCD_COL1_HEADER,y, "Pitch")
    lcd.font(FONT_STD)
    showNumberBoxed(x_data1,y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[1]) -- Pitch G
    showNumberBoxed(x_data2,y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[4]) -- Ritch H 

    y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
    lcd.font(FONT_BOLD)
    lcd.drawText (LCD_COL1_HEADER,y, "Yaw", TEXT_SIZE)
    lcd.font(FONT_STD)
    showNumberBoxed(x_data1,y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[2]) -- Yaw G
    showNumberBoxed(x_data2,y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[5]) -- Yaw H 
  elseif (page==2) then
    lcd.font(FONT_BOLD)
    ...
    lcd.font(FONT_STD)
    showNumberBoxed(x_data1, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[6]) 

    lcd.font(FONT_BOLD)
    lcd.drawText (x_head2,y, "Roll R")
    lcd.font(FONT_STD)
    showNumberBoxed(x_data2, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[12])

    y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
    lcd.font(FONT_BOLD)
    lcd.drawText (x_head1,y, "Pitch")
    lcd.font(FONT_STD)
    showNumberBoxed(x_data1, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[7]) 

    lcd.font(FONT_BOLD)
    lcd.drawText (x_head2,y, "Roll L" )
    lcd.font(FONT_STD)
    showNumberBoxed(x_data2, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[13])

    y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
    lcd.font(FONT_BOLD)
    lcd.drawText (x_head1,y, "Yaw")
    lcd.font(FONT_STD)
    showNumberBoxed(x_data1, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[8]) 
  
    lcd.font(FONT_BOLD)
    lcd.drawText (x_head2,y, "Pitch U")
    lcd.font(FONT_STD)
    showNumberBoxed(x_data2, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[14])

    y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
    lcd.font(FONT_BOLD)
    lcd.drawText (x_head2,y, "Pitch D")
    lcd.font(FONT_STD)
    showNumberBoxed(x_data2, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.AS3X_Data[15])
  end
end
...
function SmartESC.paint()
  ...
  for i=1,9 do
      ...
      lcd.font(FONT_STD)
      showNumberBoxed(x_data, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, val, this.ESC_uom[i]) 
      showNumberBoxed(x_data2, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, min, this.ESC_uom[i]) 
      showNumberBoxed(x_data3, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, max, this.ESC_uom[i])
...
function SmartBat.paint()
  ...
  local yOrig = form.height() + LCD_LINE_HEIGHT
  ...
  for i=1,6 do
      ...
      showNumberBoxed(x_data, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, this.BAT_Values[i], this.BAT_uom[i]) 
      y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
  end
  ...
  for i=0,8 do
      if ((this.BAT_Cells[i] or 0) > 0) then
        ...
        lcd.font(FONT_STD)
        showNumberBoxed(x_data, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, string.format("%2.2f",this.BAT_Cells[i]), "v") 
      end
      y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
  end
end
...
function TextGen.paint()
  ...
  if (this.TG_Lines[0]) then   -- Header
    lcd.drawText (LCD_X_MAX // 2, y, this.TG_Lines[0], TEXT_CENTERED)
  end

  y = form.height() + LCD_LINE_HEIGHT // 2
  -- Menu lines
  lcd.font(FONT_STD)
  for i=1,8 do
...
function FlightPack.paint()
  ...
  for i=0,3 do
    ...
    lcd.font(FONT_STD)
    showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, disp1, this.FP_uom[i]) 
    showNumberBoxed(LCD_COL2_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, disp2, this.FP_uom[i]) 
    y = y + LCD_LINE_HEIGHT + LCD_LINE_PADDING
  end -- For
end
...
function Gps.paint()
  ...
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W*3, LCD_LINE_HEIGHT, string.format("%f (%s)",this.GPS_Lat/1000000,this.GPS_Lat_Str))
  y = y + LCD_LINE_HEIGHT

  lcd.font(FONT_BOLD)
  lcd.drawText (LCD_COL1_HEADER, y, "LON")
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W*3, LCD_LINE_HEIGHT, string.format("%f (%s)",this.GPS_Lon/1000000,this.GPS_Lon_Str))
  y = y + LCD_LINE_HEIGHT

  lcd.font(FONT_BOLD)
  lcd.drawText (LCD_COL1_HEADER, y, "Speed")
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, string.format("%3.1f",this.GPS_Speed), "kts")
  ...
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT,string.format("%3.1f",this.GPS_Alt + this.GPS_AltHigh*1000), "f")
  y = y + LCD_LINE_HEIGHT

  lcd.font(FONT_BOLD)
  lcd.drawText (LCD_COL1_HEADER, y, "Course")
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT, string.format("%3.1f",this.GPS_Course), "deg")
  y = y + LCD_LINE_HEIGHT

  lcd.font(FONT_BOLD)
  lcd.drawText (LCD_COL1_HEADER, y, "Time")
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W*2, LCD_LINE_HEIGHT, this.GPS_Time_UTC, "(UTC)")
  y = y + LCD_LINE_HEIGHT

  lcd.font(FONT_BOLD)
  lcd.drawText (LCD_COL1_HEADER, y, "Sats")
  lcd.font(FONT_STD)
  showNumberBoxed(LCD_COL1_DATA, y, LCD_NUMBER_W, LCD_LINE_HEIGHT,this.GPS_Sats)
  y = y + LCD_LINE_HEIGHT
end
...
function MainScreen.init()
  ...
  local buttonHeight = LCD_LINE_HEIGHT * 2
  local buttonWidth = LCD_X_MAX / 4
  local buttonHeightPadding = LCD_LINE_HEIGHT // 4
...
local function create()
  ...
  local tw, th = lcd.getTextSize("99999999999999")
  LCD_LINE_HEIGHT  = th + 5

  local cuadrant = LCD_X_MAX // 4
  ...
  LCD_COL1_DATA = LCD_COL1_HEADER + tw
  LCD_COL2_DATA = LCD_COL2_HEADER + tw
